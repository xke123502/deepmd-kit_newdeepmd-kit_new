...
# here defined the repflows
        ...
        # 初始化为tensor以避免JIT类型推断问题
        self.last_mad_gap = torch.tensor(0.0, dtype=self.prec)
    ...
    def forward(
        ...
        # else: 不更新last_mad_gap，保持之前的tensor值
        return (
    ...
    def _compute_mad(self, node_ebd: torch.Tensor) -> torch.Tensor:
        ...
        node_ebd_norm = F.normalize(node_ebd, p=2.0, dim=-1)  # [nf, nloc, embed_dim]
