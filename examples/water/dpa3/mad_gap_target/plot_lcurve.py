...
def plot_training_curves(data, save_path=None):
    ...
    # 1. MAD Gap值
    ax1 = axes[0]
    ax1.plot(data['step'], data['mad_value_val'], 'b-', label='Validation', linewidth=1.5)
    ax1.plot(data['step'], data['mad_value_trn'], 'r-', label='Training', linewidth=1.5)
...
def print_data_summary(data):
    ...
    print(f"  MAD Gap (val/trn): {final_row['mad_value_val']:.6f} / {final_row['mad_value_trn']:.6f}")
